...
const createnew = (body) => {
  ...
  return dbPool.execute(SQLQuery, [body.title, body.writer, body.publisher, body.year, body.user_id, body.category_id]);
};

const updateUser = (body, id) => {
  const SQLQuery = `UPDATE book SET title = ?, writer = ?, publisher = ?, year = ?, user_id = ?, category_id = ? WHERE id = ?`;
  return dbPool.execute(SQLQuery, [body.title, body.writer, body.publisher, body.year, body.user_id, body.category_id, id]);
};
