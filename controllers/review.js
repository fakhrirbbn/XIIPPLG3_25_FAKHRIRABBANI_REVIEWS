...
module.exports = {
  ...
  post: (req, res) => {
    ...
    db.query('INSERT INTO reviews (book_id, user_id, rating, comment, created_at) VALUES (?, ?, ?, ?, ?)', [book_id, user_id, rating, comment, created_at], (err, result) => {
      if (err) {
        return res.status(500).json({ error: err.message }); // Tangani error database
      }
  ...
  put: (req, res) => {
    ...
    db.query('UPDATE reviews SET book_id = ?, user_id = ?, rating = ?, comment = ?, created_at = ? WHERE id = ?', [book_id, user_id, rating, comment, updated_at, reviewId], (err, result) => {
      if (err) {
        return res.status(500).json({ error: err.message }); // Tangani error database
      }
